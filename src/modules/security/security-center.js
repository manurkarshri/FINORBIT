...
export function createSecurityCenter({ onSetup, onLock, onStandardBackup, onEncryptedBackup, onBackupStatus, onRestorePreview, onReset, onStorageStatus, onPersistStorage, onDatabaseHealth }) {
  ...
  const refreshBackupStatus = async () => { const last = await onBackupStatus?.(); backupStatus.textContent = last ? `Last downloaded backup: ${new Date(last).toLocaleString()}. Keep the file outside this browser.` : "No backup has been recorded in this browser yet. Download an encrypted backup now."; };
  ...
  const includeReceipts = node("input", { type: "checkbox", id: "backup-include-receipts" });
  const receiptOption = node("label", { for: "backup-include-receipts", text: " Include receipt files in encrypted backup" }); receiptOption.prepend(includeReceipts);
  ...
  encryptedButton.addEventListener("click", async () => {
    ...
    if (passphrase) { downloadJson(await onEncryptedBackup(passphrase, { includeReceipts: includeReceipts.checked }), `finorbit-encrypted-${new Date().toISOString().slice(0, 10)}.json`); await refreshBackupStatus(); }
  });
  ...
  const center = node("section", { class: "security-center", "aria-labelledby": "security-title" }, [
    ...
    node("p", { text: "FinOrbit cannot silently create or continuously update a permanent folder on every phone. A downloaded backup is the only copy that survives removal of this site's browser data." }),
    backupStatus, receiptOption,
    node("div", { class: "security-center__actions" }, [backupButton, encryptedButton, restoreInput, resetButton]), status,
