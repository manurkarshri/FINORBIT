...
const LABELS = { investment: "Investment", property: "Property", vehicle: "Vehicle" };
...
export function createWealthCenter({ wealth, entities, liveRegion }) {
  ...
  function valuationEditor(choices, today) {
    ...
    if (!choices.length) { submit.disabled = true; section.append(element("p", "Add an active investment, property, or vehicle before recording a valuation.", "wealth-warning")); }
  ...
  async function render() {
    ...
    try {
      ...
      const header = element("header", null, "wealth-center__header"); header.append(element("p", "Your complete position", "route-heading__eyebrow"), element("h1", "Wealth"), element("p", `Calculated through ${asOfDate}. Values stay on this device.`));
      ...
      const cards = element("div", null, "wealth-summary"); for (const [label, value, primary] of [["Net worth", snapshot.netWorthPaise, true], ["Assets", snapshot.totalAssetsPaise], ["Liabilities", snapshot.totalLiabilitiesPaise], ["This month’s savings", snapshot.savingsPaise]]) { const card = element("article", null, primary ? "wealth-card wealth-card--primary" : "wealth-card"); card.append(element("p", label), element("strong", formatMoney(value))); cards.append(card); }
      const explanation = element("section", null, "wealth-explanation"); explanation.append(element("h2", "Wealth change explanation")); const rows = [["Net-worth change", snapshot.explanation.netWorthChangePaise], ["Income contribution", snapshot.explanation.incomeContributionPaise], ["Expense impact", snapshot.explanation.expenseImpactPaise], ["Investment contributions", snapshot.explanation.investmentContributionPaise], ["Market-value change", snapshot.explanation.marketValueChangePaise], ["Debt reduction", snapshot.explanation.debtReductionPaise], ["Loan interest", -snapshot.explanation.loanInterestPaise], ["Asset depreciation", snapshot.explanation.assetDepreciationPaise], ["Adjustments", snapshot.explanation.adjustmentsPaise], ["Other capital movement", snapshot.explanation.otherCapitalChangePaise]]; const dl = element("dl", null, "wealth-breakdown"); for (const [label, value] of rows) { dl.append(element("dt", label), element("dd", formatMoney(value))); } explanation.append(dl, element("p", `Income ${formatMoney(snapshot.incomePaise)} · Expenses ${formatMoney(snapshot.expensePaise)} · Savings rate ${snapshot.savingsRateBasisPoints == null ? "not available" : `${(snapshot.savingsRateBasisPoints / 100).toFixed(1)}%`}.`));
      ...
      if (snapshot.excludedEntityCount) explanation.append(element("p", `${snapshot.excludedEntityCount} entit${snapshot.excludedEntityCount === 1 ? "y is" : "ies are"} excluded by your Include in net worth settings.`));
