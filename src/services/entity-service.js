...
export const ENTITY_CONFIG = Object.freeze({
  ...
  property: { store: "properties", name: "nickname", statuses: ["active", "sold", "archived"], money: ["purchasePricePaise", "openingEstimatedValuePaise"], required: ["nickname", "propertyType", "ownershipPercentage", "openingEstimatedValuePaise", "valuationDate", "includeInNetWorth", "status"] },
  vehicle: { store: "vehicles", name: "nickname", statuses: ["active", "sold", "archived"], money: ["purchasePricePaise", "openingEstimatedValuePaise"], required: ["nickname", "vehicleType", "openingEstimatedValuePaise", "valuationDate", "includeInNetWorth", "status"] },
});
...
export function createEntityService(database, { now = () => new Date().toISOString(), id = () => createOpaqueId() } = {}) {
  async function save(type, input, { source = "manual" } = {}) {
    ...
    const stores = [config.store, "auditLogs", ...(opening != null ? ["openingPositions"] : [])];
...
function openingValue(type, record) { return ({ account: record.openingBalancePaise, creditCard: record.openingOutstandingPaise, loan: record.outstandingPrincipalPaise, investment: record.openingEstimatedValuePaise, property: record.openingEstimatedValuePaise, vehicle: record.openingEstimatedValuePaise })[type]; }
